      ...
      {stage === 'confirm' && (
        <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-black/70 p-4" onClick={reset}>
          <div className="card-cream w-full max-w-sm p-6 text-center" onClick={(e) => e.stopPropagation()}>
            ...
            <h3 className="mt-2 font-display text-xl font-bold text-night">{t('sos.confirmTitle')}</h3>
            <p className="mt-1 text-sm text-night/60">{t('sos.confirmBody')}</p>
            <div className="mt-5 flex gap-2">
              <button onClick={reset} className="flex-1 rounded-xl border-2 border-night/15 py-2.5 text-sm font-semibold text-night">
                {t('sos.cancel')}
              </button>
        ...
        <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-black/70 p-4" onClick={reset}>
          <div className="card-cream w-full max-w-sm p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between">
              ...
              <button onClick={reset} className="text-night/40 hover:text-night">✕</button>
            </div>

            {stage === 'locating' ? (
              <p className="mt-4 animate-pulse text-sm text-night/60">{t('sos.locating')}</p>
            ) : (
              <div className="mt-4 space-y-3">
                ...
                {nearest && (
                  <div className="rounded-xl border border-night/10 p-3">
                    <p className="text-xs font-semibold uppercase tracking-wide text-night/40">{t('sos.nearestPolice')}</p>
                    <p className="mt-0.5 text-sm font-semibold text-night">👮 {nearest.name}</p>
                    <p className="text-xs text-night/60">{nearest.phone} · {(nearestDistM / 1000).toFixed(1)} km</p>
                  </div>
                )}

                {shared ? (
                  <p className="rounded-xl bg-emerald/15 px-3 py-2 text-center text-sm font-semibold text-emerald">
                ...
                <p className="text-center text-[10px] text-night/40">{t('sos.demoNote')}</p>
              </div>
            )}
          </div>
        </div>
