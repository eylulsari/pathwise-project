    ...
    <div className="fixed inset-0 z-[1100] flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      <div className="card-cream w-full max-w-sm p-6" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between">
          <div>
            <h3 className="font-display text-lg font-bold text-night">📸 {t('journal.title')}</h3>
        ...
        <div className="mt-4 flex justify-center gap-1">
          {[1, 2, 3, 4, 5].map((n) => (
            <button
              key={n}
              onClick={() => setRating(n)}
              className={`text-3xl ${n <= rating ? 'text-coral' : 'text-night/20'}`}
              aria-label={`${n} stars`}
            >
              ★
            </button>
        ...
        <label className="mt-4 block text-sm font-semibold text-night/80">{t('journal.photo')}</label>
        <input
          value={photoUrl}
        ...
        <label className="mt-3 block text-sm font-semibold text-night/80">{t('journal.note')}</label>
        <textarea
          value={note}
          onChange={(e) => setNote(e.target.value)}
          rows={3}
          placeholder={t('journal.notePlaceholder')}
          className="mt-1 w-full resize-none rounded-lg border border-night/15 px-3 py-2 text-night outline-none focus:border-violet"
        />

        <button onClick={save} disabled={busy} className="btn-accent mt-4 w-full py-2 text-sm">
          {busy ? t('journal.saving') : t('journal.save')}
        </button>
        <p className="mt-2 text-center text-[10px] text-night/40">{t('journal.mockNote')}</p>
      </div>
    </div>
  );
}
