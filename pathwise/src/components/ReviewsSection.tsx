...
export function ReviewsSection({ place }: { place: Place }) {
  ...
  return (
    <section className="mt-4 border-t border-night/10 pt-4">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-bold text-violet-deep">💬 {t('reviews.title')}</h4>
        {reported ? (
          <span className="text-[10px] font-semibold text-emerald">✓ {t('reviews.reported')}</span>
        ) : (
          <button onClick={reportStale} className="text-[10px] font-semibold text-night/40 hover:text-fuchsia">
            🚩 {t('reviews.stale')}
          </button>
        )}
      </div>
      ...
      <p className="mt-1 text-xs text-night/70">
        ...
        {data && data.count > 0 && (
          <>
            {' · '}
            <span className="font-semibold text-violet-deep">{t('reviews.community')}:</span> {data.average}★ ({data.count})
          </>
        )}
      </p>

      {/* Write a review */}
      <div className="mt-3 rounded-xl bg-night/5 p-3">
        <div className="flex gap-1">
          {[1, 2, 3, 4, 5].map((n) => (
            <button
              key={n}
              onClick={() => setRating(n)}
              className={`text-xl ${n <= rating ? 'text-coral' : 'text-night/20'}`}
              aria-label={`${n} stars`}
            >
              ★
            </button>
        ...
        <textarea
          ...
          className="mt-2 w-full resize-none rounded-lg border border-night/15 px-3 py-2 text-sm text-night outline-none focus:border-violet"
        />
      ...
      <div className="mt-3 space-y-2">
        {data && data.reviews.length === 0 && (
          <p className="text-xs text-night/50">{t('reviews.empty')}</p>
        )}
        {data?.reviews.map((r) => (
          <div key={r.id} className="rounded-lg border border-night/10 p-2.5">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-night">{r.authorName}</span>
              <span className="text-xs text-coral">{'★'.repeat(r.rating)}</span>
            </div>
            <p className="mt-1 text-sm text-night/80">{r.comment}</p>
            <button onClick={() => helpful(r.id)} className="mt-1 text-[11px] font-semibold text-violet hover:text-fuchsia">
