    ...
    <div className="rounded-2xl border border-white/10 bg-night-800 p-4">
      ...
      <div className="space-y-1.5">
        {SURVIVAL_GUIDE.map((cat) => {
          ...
          return (
            <div key={cat.id} className="overflow-hidden rounded-lg border border-white/10">
              <button
                onClick={() => setOpen(isOpen ? null : cat.id)}
                className="flex w-full items-center justify-between px-3 py-2 text-left text-sm font-semibold text-cream/90 hover:bg-white/5"
              >
                <span>{cat.icon} {cat.title}</span>
                <span className={`transition-transform ${isOpen ? 'rotate-180' : ''}`}>⌄</span>
              </button>
              {isOpen && (
                <ul className="space-y-1.5 px-3 pb-3 pt-1 text-xs text-cream/70">
                  {cat.tips.map((t, i) => (
